...
- 兩邊都空白：不清除本機既有內容。
- `續訂/停訂`：來源有值且與本機不同時同步更新；來源空白不清除本機既有狀態。本機值為 `OK`（不分大小寫）時一律保留，不被來源的「續訂」或「停訂」覆蓋。
...
`.xlsx` 是 ZIP 封裝。本工具只替換目標工作表 XML 中明確指定的 `上個月回覆`與`續訂/停訂`儲存格；不再重新序列化整張工作表，未指定的欄位與其他 ZIP 內容保持原始位元，避免樞紐分析、格式、公式或文字索引被連帶改動。
